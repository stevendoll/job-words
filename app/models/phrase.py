...
class Phrase(db.Model):
    ...
    def mean_salary(cls):
        return (
            select([func.sum(Finding.mean_salary)])
    ...
    def jobs_count(cls):
        return (
            select([func.sum(Finding.jobs_count)])
            .where(Finding.phrase_id == cls.id)
            .as_scalar()
        )
    ...
    def jobs_above_100k_count(cls):
        return (
            select([func.sum(Finding.jobs_above_100k_count)])
            .where(Finding.phrase_id == cls.id)
            .as_scalar()
        )
    ...
    def serialize(self):
        ...
        result["createdDate"] = (
            self.created_date.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            if isinstance(self.created_date, dt.date)
            else None
        )
        result["updatedDate"] = (
            self.updated_date.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            if isinstance(self.updated_date, dt.date)
            else None
        )
    ...
    @staticmethod
    def get_all():
        return Phrase.query.filter(Phrase.jobs_count > PHRASE_MINIMUM_JOBS).order_by(
    ...
    def get_last():
        return (
            Phrase.query.filter(Phrase.jobs_count > PHRASE_MINIMUM_JOBS)
